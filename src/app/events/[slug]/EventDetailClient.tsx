          ...
          {booked ? (
            ...
            <>
              ...
              <Button
                ...
                disabled={!name.trim() || seatsAvailable <= 0 || seats > seatsAvailable || booking}
                onClick={() => {
                  void (async () => {
                    setBooking(true);
                    setError("");
                    const ok = await bookSeats(event.id, seats);
                    setBooking(false);
                    if (!ok) {
              ...
              >
                {seatsAvailable <= 0 ? "Sold out" : booking ? "Reserving…" : "Reserve"}
              </Button>
            </>
          )}
        </div>
